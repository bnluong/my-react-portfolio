...
const ProjectModal = ({ show, closeModal, project }) => {
	return (
		<div
			className='project-modal-wrapper fixed top-0 bottom-0 left-0 right-0 transition-all duration-300 ease-in'
			style={{
		...
		>
			<div
				className='project-modal-backdrop fixed top-0 left-0 w-full h-full z-10 bg-black bg-opacity-80'
				onClick={closeModal}
			></div>
			...
			<div className='fixed z-20 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 mt-6 max-w-full max-h-full w-5/6 md:h-auto sm:h-3/5 h-3/4'>
				<div className='project-modal-content top-0 left-0 max-w-full max-h-full overflow-y-auto flex flex-col bg-white p-3 border-2 border-separate sm:shadow-xl rounded-lg'>
